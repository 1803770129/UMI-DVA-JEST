...
export default props => {
    ...
    const columns = [
        ...
        {
            title: '启用状态',
            dataIndex: 'fms_part_status',
            width: 250,
            render: (text, record, index) => (
                <Switch 
                    checkedChildren="开" 
                    unCheckedChildren="关" 
                    defaultChecked={record.fms_part_status} 
                    checked={record.fms_part_status}
                    onChange={checked => {onChangePartStatusFn(record.fms_category_id, record.fms_part_id, checked);}} 
                />
            )
        },
        {
            ...
            dataIndex: 'operation',
            width: 200,
            render: (text, record, index) => <span className="blue6 cur" onClick={() => {onGoFmsCategoryDetailFn(record.category_id, record.fms_brand_id, record.fms_category_id, record.fms_part_id);}}>产品管理</span>
        }
    ];
    ...
    const TableTitle = categoryInfo => {
        return (
            <Row type="flex" justify="space-between">
                <Col>
                    <strong>品牌：</strong>{categoryInfo.fms_brand_name} &emsp; <strong>品类：</strong>{categoryInfo.brand_category_name}
                </Col>
                <Col>
                    启用状态：
                    <Switch
                        checkedChildren="开"
                        unCheckedChildren="关"
                        defaultChecked={categoryInfo.fms_category_status}
                        checked={categoryInfo.fms_category_status}
                        onChange={checked => {onChangeCategoryStatusFn(categoryInfo.fms_category_id, checked);}}
                    />
                </Col>
            </Row>
        );
        ...
        <div className="factorycode-category">
            ...
            {/* 表格 */}
            {
                ...
                fmsCategoriesList.list.map(item => {
                    return (
                        <Table
                            key={item.categoryInfo.fms_category_id}
                            {...tableProps}
