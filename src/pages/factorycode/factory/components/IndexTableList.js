...
export default ({ loading, searchFields, fmsBrandsList, onHandlePageSizeChangeFn, onHandlePageChangeFn, onGoFmsBrandDetailFn, onDeleteFmsBrandFn }) => {
    ...
    const columns = [
        { title: '大厂品牌', dataIndex: 'fms_brand_name' },
        { title: '大厂品牌描述', dataIndex: 'fms_brand_desc' },
        { title: '配件品类', dataIndex: 'categoryNames' },
        { title: '优先级', dataIndex: 'fms_brand_level' },
        { title: '启用状态', dataIndex: 'fms_brand_status', width: 80 },
        { title: '创建时间', dataIndex: 'create_time', width: 160 },
        {
            ...
            render: (text, record) => (
                <React.Fragment>
                    <span className="blue6 cur m-r-15" onClick={() => {onGoFmsBrandDetailFn(record.fms_brand_id);}}>编辑</span>
                    <span className="blue6 cur" onClick={() => {onDeleteFmsBrandFn(record.fms_brand_id);}}>删除</span>
                </React.Fragment>
            )
        }
    ];
