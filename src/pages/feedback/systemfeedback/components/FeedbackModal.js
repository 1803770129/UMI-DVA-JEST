...
const ReplyModal = ({ loading, form, visible, reply_imgs, cur_row, list, onOk, onCancel, onUploadPic, onUpdatePic, onReply, onPreviewImage }) => {
  ...
  // 初始化处理状态
  const init_feedback_status = (cur_row.sys_feedback_status === 'PENDING' || !cur_row.sys_feedback_status) ? 'PROCESSING' : cur_row.sys_feedback_status;
  
  let cur_row_imgs = cur_row.sys_feedback_images || [];
  cur_row_imgs = cur_row_imgs.map(img => img.sys_feedback_image_url).filter(url => !!url);
  return (
    <Modal destroyOnClose title="反馈处理" width={650} visible={visible} footer={null} onCancel={onCancel} className={styles.reply_modal}>
      <div className={styles.reply_content} id="system_reply_content">
        {/* 反馈行 */}
        <div className={styles.issue}>
          <div className={styles.issue_title}>问题对象</div>
          <div className={styles.issue_desc}>{cur_row.app_name}</div>
          <div className={styles.issue_title}>问题描述</div>
          <div className={styles.issue_desc}>{cur_row.sys_feedback_content}</div>
          {
          ...
          <div className={styles.issue_bt}>
            <span>{cur_row.person_name} ({cur_row.sys_feedback_phone})</span><span className="m-l-15">{cur_row.sys_feedback_time}</span>
          </div>
        </div>
        ...
        {
          list.map((v, vIdx) => {
            // sys_feedback_process_content 反馈单处理内容(sys_feedback_process_stance > USER); 
            // sys_feedback_content 反馈内容 (sys_feedback_process_stance > SOPEI)
            const { sys_feedback_process_stance, sys_feedback_process_content, sys_feedback_images = [], person_name, sys_feedback_process_time } = v;
            const imgs = sys_feedback_images.map(img => img.sys_feedback_image_url).filter(url => !!url);
            
            return (
              <div key={vIdx}>
                <div className={styles.issue}>
                  <div className={styles.issue_desc}>{sys_feedback_process_content}</div>
                  {
                    imgs.length > 0 && 
                    <div className={styles.issue_pic} style={{ marginBottom: sys_feedback_process_stance === 'SOPEI' ? 10 : 0 }}>
                      {
                        imgs.map((img, imgIdx) => {
                  ...
                  <div className={styles.issue_bt}>
                    <span>{person_name}</span><span className="m-l-15">{sys_feedback_process_time}</span>
                  </div>
                </div>
      ...
      <div className="m-t-10">
        <FormItem className="rel" style={{ marginBottom: 0 }}>
          ...
          <Button type="default" onClick={handleSubmit} loading={loading['systemFeedback/fetchFeedbackReply']} className={styles.reply_btn}>回复</Button>
        </FormItem>
