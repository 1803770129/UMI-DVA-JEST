...
const emptyFields = {
    page: 1,                                // 当前页数
    perpage: 10,                            // 每页多少条
    indus_brand_status: 'ENABLE',             // 启用状态
    indus_brand_id: ''                        // 大厂品牌
};

export default {
    namespace: 'indus_list',
    ...
    effects: {
        ...
        *fetchFmsBrandsList({ payload }, { call, put, select }) {
            ...
            if(res.code === 0) {
                ...
                const { searchFields } = yield select(state => state.indus_parts_id);
                yield put({ 
                    type: 'saveFmsBrandsParamsFn', 
                    payload: {...searchFields, page: payload.page, perpage: payload.perpage, indus_brand_id: payload.indus_brand_id, indus_brand_status: payload.indus_brand_status} 
                });
        ...
        *deleteFmsBrand({ payload }, { call, put, select }) {
            const res = yield call(request, { 
                fnName: 'indus_brands_del', 
                data: { indus_brand_id: payload.indus_brand_id }
            });
            if(res.code === 0) {
                msg('删除成功');
                const { fmsBrandsList } = yield select(state => state.indus_list);
                // 手动删除表格数据
                let newList = fmsBrandsList.list.filter(item => item.indus_brand_id != payload.indus_brand_id);
