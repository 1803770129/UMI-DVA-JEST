      ...
      if (isArray(row[key])) {
        ...
        if(row[key].length > 0) {
          ...
          for (let i = 0; i < imgs.length; i++) {
            const img = imgs[i];
            imgStr += `<span><img src=${img} width="${width}" height=${height} /></span>`
          }
        }
        tbody +=
          `<td style="width: ${width}px; height: ${height + 15}px; text-align: center; vertical-align: middle;">
            ${imgStr}
          </td>
          `;
        ...
        // 增加&nbsp; 目的是消除字符串数字会被excel用科学计数法转换
        const str = row[key] ? (row[key] + '&nbsp;') : '';
        tbody += '<td style="text-align:center">' + str + '</td>';
      }
    }
    tbody += '</tr>';
  }
